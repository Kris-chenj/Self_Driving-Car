...
namespace tk
{
    ...
    public class TcpCarHandler : MonoBehaviour {
        ...
        void Awake()
        {
            ...
            Canvas Avoidance_Canvas = GameObject.FindObjectOfType<Canvas>();
            GameObject go1 = CarSpawner.getChildGameObject(Avoidance_Canvas.gameObject, "AvoidanceAISteering");
            if (go1 != null)
                Avoidanceai_text = go1.GetComponent<Text>();
        }
        ...
        {    
            ...
            else if(state == State.SendTelemetry)
            {
                ...
                if (Avoidanceai_text != null)
                    Avoidanceai_text.text = string.Format("Model steer: {0}\nthrottle: {1}\nspeed: {2}", ai_steering, ai_throttle, ai_speed);
            }
        }
    }
}
